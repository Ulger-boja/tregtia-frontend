...
export default function ListingDetail() {
  ...
  if (!listing) return <div className="text-center py-20 text-gray-400">Not found</div>;

  const isFav = isFavorite(listing.id);
  const similar = MOCK_LISTINGS.filter(l => l.category?.slug === listing.category?.slug && l.id !== id).slice(0, 4);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 text-sm text-gray-400">
          ...
          <span>/</span>
          <Link to={`/listings/${listing.category?.slug}`} className="hover:text-primary-600">
            {isEn ? listing.category?.name_en : listing.category?.name_sq}
          </Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl overflow-hidden border border-gray-100">
              <div className="relative aspect-[16/10] bg-gray-100 cursor-pointer" onClick={() => setLightbox(0)}>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => isAuthenticated && toggleFavorite(listing.id)} className={`p-2 rounded-full ${isFav ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400 hover:text-red-500'}`}>
                    <Heart size={20} className={isFav ? 'fill-current' : ''} />
                  </button>
                  <button onClick={handleShare} className="p-2 rounded-full bg-gray-50 text-gray-400 hover:text-primary-600">
                    <Share2 size={20} />
                  </button>
                </div>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="flex items-center gap-3 mb-4">
                <img src={listing.user?.avatar} alt="" className="w-12 h-12 rounded-full object-cover" />
                <div>
                  <p className="font-semibold text-gray-900">{listing.user?.name}</p>
                  <p className="text-xs text-gray-400 flex items-center gap-1">
                    <User size={11} />
                    {isEn ? 'Member since' : 'Anëtar që nga'} {new Date(listing.user?.createdAt).getFullYear()}
                  </p>
                </div>
              </div>

              {showPhone ? (
                <a href={`tel:${listing.user?.phone}`} className="w-full flex items-center justify-center gap-2 bg-emerald-500 text-white py-3 rounded-xl font-medium hover:bg-emerald-600 transition mb-2">
                  <Phone size={18} />
                  {listing.user?.phone}
                </a>
              ) : (
              ...
              <button onClick={() => toast.success(isEn ? 'Coming soon!' : 'Së shpejti!')} className="w-full flex items-center justify-center gap-2 bg-primary-600 text-white py-3 rounded-xl font-medium hover:bg-primary-700 transition">
                ...
                {isEn ? 'Send Message' : 'Dërgo mesazh'}
              </button>
            </div>

            {/* Report */}
            <button className="w-full flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-red-500 py-2">
              <Flag size={14} />
              {isEn ? 'Report this ad' : 'Raporto njoftimin'}
            </button>
          </div>
        </div>
